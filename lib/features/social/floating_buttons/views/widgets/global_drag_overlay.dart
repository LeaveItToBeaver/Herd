...
class GlobalDragOverlay extends ConsumerStatefulWidget {
  final BubbleConfigState config;
  final dynamic appTheme;
  final Offset startPosition;
  final VoidCallback onDragEnd;
  final GlobalDraggableNotifier dragStateProvider;
  ...
  const GlobalDragOverlay({
    ...
    required this.dragStateProvider,
  });

  @override
  ConsumerState<GlobalDragOverlay> createState() => _GlobalDragOverlayState();
}

class _GlobalDragOverlayState extends ConsumerState<GlobalDragOverlay>
  ...
  void initState() {
    ...
    _snapBackAnimation.addListener(() {
      ...
      widget.dragStateProvider.updateDrag(_currentPosition);
    });
        ...
        onPanUpdate: (details) {
          setState(() {
            ...
            _currentPosition = Offset(
              _currentPosition.dx
                  .clamp(0, screenSize.width - widget.config.effectiveSize),
              _currentPosition.dy
                  .clamp(0, screenSize.height - widget.config.effectiveSize),
            );
          });
          // Update global state
          widget.dragStateProvider.updateDrag(_currentPosition);
